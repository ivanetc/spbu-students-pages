...
2. [Валидация входящих данных](#p1.5)
3. [Аутентификация и авторизация](#p2)
4. [JWT-аутентификация](#p3)
5. [Как реализовать аутентификацию в Spring?](#p4)
...
![](../../resources/lectures/2025-spring/lec09/p1.png)

### Валидация входящих данных <a name="p1.5"></a>

#### 1. Мотивация (проблема)

Пользователи могут отправлять невалидные данные, что приводит к различным проблемам:
- **Баги и падения приложения**: неправильные данные могут вызвать исключения в бизнес-логике
- **Проблемы безопасности**: злоумышленники могут использовать невалидные данные для атак
- **Некорректная работа системы**: приложение может работать неправильно с невалидными данными

Примеры невалидных данных:
- Пустые обязательные поля
- Неправильный формат email
- Отрицательные значения там, где нужны положительные
- Строки длиннее допустимого предела

#### 2. Где происходит валидация

Валидация происходит в цепочке обработки запроса после Data Binding и до вызова контроллера:
![](../../resources/lectures/2025-spring/lec09/p8.png)

Client → Filter → DispatcherServlet → Data Binding → **Validation** → Interceptor → Controller

Процесс:
1. DispatcherServlet анализирует запрос и определяет контроллер
2. Происходит Data Binding - преобразование параметров запроса в объекты Java (DTO)
3. **Валидация** - проверка корректности данных в DTO
4. Только после успешной валидации вызывается метод контроллера

#### 3. Как реализуется в Spring

Spring использует Bean Validation API (JSR-380) с аннотациями:
...
В контроллере используется аннотация `@Valid`:
...
- **Контроллер НЕ вызывается** - метод не выполняется
- **Возвращается HTTP 400 (Bad Request)**
- **Выбрасывается исключение** MethodArgumentNotValidException
- Spring автоматически создает детализированный ответ с ошибками

#### 5. Связь с фильтрами и интерсепторами

##### Валидация vs Interceptor
- **Общее**: оба выполняются до контроллера
- **Различие**:
  - Validation - проверка корректности данных
  - Interceptor - управление поведением (логирование, авторизация)

##### Валидация vs Filter
- **Filter раньше**: работает на уровне Servlet, до DispatcherServlet
- **Filter не знает про DTO**: работает с сырыми HTTP-запросами
- **Не подходит для бизнес-валидации**: не имеет контекста Spring

##### Валидация vs Security
- **Validation**: корректность данных (что отправлено)
- **Authentication**: кто ты (проверка личности)
- **Authorization**: что тебе можно (проверка прав)
...
![](../../resources/lectures/2025-spring/lec09/p2.png)
...
![](../../resources/lectures/2025-spring/lec09/p3.png)
...
![](../../resources/lectures/2025-spring/lec09/p4.png)
...
![](../../resources/lectures/2025-spring/lec09/p5.png)
...
![](../../resources/lectures/2025-spring/lec09/p6.png)
...
![](../../resources/lectures/2025-spring/lec09/p7.png)
